...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    if (isMobile) {
      // On mobile, make the plane nearly fill the container
...
class App {
  ...
  onCheck() {
    ...
    if (this.onActiveItemChange) {
      // Use modulo to wrap around to the original items array length
      const originalLength = this.mediasImages.length / 2;
      const activeIdx = itemIndex % originalLength;
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, onActiveItemChange, disableTitle]);
  return (
    <div
